...
(function(angular) {
  ...
  angular.module("door3.css", []).run(function() {
    console.error(
      'AngularCSS: The module name "door3.css" is now deprecated. Please use "angularCSS" instead.'
    );
  });

  // Provider
  angularCSS.provider("$css", [
    function $cssProvider() {
      ...
      this.$get = [
        ...
        function $get(
          ...
          var template =
            '<link ng-repeat="stylesheet in stylesheets | orderBy: \'weight\' track by $index " rel="{{ stylesheet.rel }}" type="{{ stylesheet.type }}" ng-href="{{ stylesheet.href }}" ng-attr-media="{{ stylesheet.media }}">';
          ...
          var mediaQuery = {},
            ...
            container = angular.element(
              document.querySelector
                ...
                : document.getElementsByTagName(options.container)[0]
            ),
            dynamicPaths = [];
          ...
          function addViaMediaQuery(stylesheet) {
            ...
            mediaQueryListener[stylesheet.href] = function(mediaQuery) {
              ...
              $timeout(function() {
                if (mediaQuery.matches) {
                  ...
                  var index = $rootScope.stylesheets.indexOf(
                    $filter("filter")($rootScope.stylesheets, {
            ...
            mediaQuery[stylesheet.href].addListener(
              mediaQueryListener[stylesheet.href]
            );
          ...
          function removeViaMediaQuery(stylesheet) {
            ...
            if (
              $rootScope &&
              angular.isDefined(mediaQuery) &&
              mediaQuery[stylesheet.href] &&
              angular.isDefined(mediaQueryListener)
            ) {
              mediaQuery[stylesheet.href].removeListener(
                mediaQueryListener[stylesheet.href]
              );
            }
          }
          ...
          function isMediaQuery(stylesheet) {
            ...
            return !!// Check for media query setting
            (
              stylesheet.media &&
          ...
          $css.getFromState = function(state) {
            ...
            if (
              ...
              (angular.isDefined(state.data) &&
                angular.isDefined(state.data.css))
            ) {
            ...
            if (!stylesheets) {
              ...
              if ($injector.has("$route")) {
                Array.prototype.push.apply(
                  stylesheets,
                  $css.getFromRoutes($injector.get("$route").routes)
                );
              }
              ...
              if ($injector.has("$state")) {
                Array.prototype.push.apply(
                  stylesheets,
                  $css.getFromStates($injector.get("$state").get())
                );
              }
            ...
            angular.forEach(stylesheets, function(stylesheet, key) {
              ...
              stylesheetLoadPromises.push(
                ...
                $http.get(stylesheet.href).error(function(response) {
                  if (DEBUG)
                    $log.error(
                      "AngularCSS: Incorrect path for " + stylesheet.href
                    );
                })
              );
            });
  ...
  angular.module = function() {
    ...
    module.directive = function(directiveName, directiveFactory) {
      var originalDirectiveFactory = angular.isFunction(directiveFactory)
        ? directiveFactory
        : directiveFactory[directiveFactory ? directiveFactory.length - 1 : 0];
      try {
        var directive = angular.copy(originalDirectiveFactory)();
        directive.directiveName = directiveName;
        if (
          directive.hasOwnProperty("css") &&
          !arrayExists(
            arraySelect($directives, function(x) {
    ...
    module.component = function(componentName, componentObject) {
      ...
      if (
        componentObject.hasOwnProperty("css") &&
        !arrayExists(
          arraySelect($directives, function(x) {
            return x.ddo.directiveName;
          }),
        ...
        angular.forEach($directives, function($dir) {
          if (!$dir.handled) {
            ...
            if ($injector.has(dirProvider)) {
              $dir.handled = true;
              $provide.decorator(dirProvider, [
                "$delegate",
                "$rootScope",
                "$timeout",
                function($delegate, $rootScope, $timeout) {
